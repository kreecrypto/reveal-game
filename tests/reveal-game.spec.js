...
async function uploadAndApplyFirstImage(page){
  ...
  await expect(page.getByRole('heading',{name:'จัดรูป'})).toBeVisible();
...
test('setup saves, reloads, and plays the custom game',async({page})=>{
  ...
  await page.getByRole('button',{name:'เล่น'}).click();
  ...
  await expect(page.getByRole('button',{name:/จบเกม|ข้อต่อไป/})).toBeVisible();
});
...
test('image editor supports zoom, crop apply, save, and re-edit',async({page})=>{
  ...
  await expect(page.locator('[data-ui="image-editor-modal"]')).not.toHaveClass(/is-hidden/);
  await expect(page.locator('[data-ui="zoom-value"]')).toHaveText('1.50×');
  await page.locator('[data-ui="image-editor-modal"]').getByRole('button',{name:'ยกเลิก'}).click();
});
...
test('dirty setup asks before leaving and can discard safely',async({page})=>{
  ...
  await expect(page.getByRole('heading',{name:'ยังไม่ได้บันทึก'})).toBeVisible();
...
test('removed question can be undone before save',async({page})=>{
  await page.goto('/setup.html');
  await page.getByRole('button',{name:'+ เพิ่มข้อ'}).click();
...
test('v21 publishes directly to Supabase and exposes only the public share link',async({page})=>{
  ...
  await page.goto('/setup.html');
  await expect(page.locator('[data-ui="cloud-status"]')).toContainText('พร้อมแชร์');
  await page.locator('#game-title').fill('เกมแชร์เทสต์');
  await fillFirstQuestion(page,{answer:'แมวแชร์'});
  await page.getByRole('button',{name:'แชร์เกม'}).click();
