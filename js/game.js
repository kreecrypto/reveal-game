(() => {
  ...
  const state={questions:[],title:'Demo 10 รูป',index:0,opened:new Set(),revealed:false,objectUrls:[]};
  const showScreen=name=>{$$('.screen').forEach(x=>x.classList.toggle('is-active',x.dataset.screen===name));window.scrollTo(0,0)};
  const normalize=data=>{if(!Array.isArray(data))throw new Error('ข้อมูลคำถามต้องเป็น array');return data.filter(x=>x&&x.image&&x.answer).map((x,i)=>({id:x.id??i+1,question:x.question||'ภาพนี้คืออะไร?',answer:String(x.answer),image:String(x.image),alt:x.alt||`ภาพคำถาม ${i+1}`}))};
  ...
  const fromCustomGame=game=>{
    ...
    const questions=game.questions.filter(x=>x?.imageBlob&&x?.answer).map((x,i)=>{
      ...
      return {id:x.id??i+1,question:x.question||'ภาพนี้คืออะไร?',answer:String(x.answer),image:url,alt:x.imageName||`ภาพคำถาม ${i+1}`};
    });
  ...
  const loadQuestions=async()=>{
    ...
    }catch(e){console.warn('โหลดชุดที่สร้างเองไม่สำเร็จ ใช้ Demo แทน',e)}
    const res=await fetch('/data/questions.json',{cache:'no-store'});
    if(!res.ok)throw new Error(`โหลดข้อมูลไม่สำเร็จ (${res.status})`);
    state.questions=normalize(await res.json());
    state.title='Demo 10 รูป';
    if(!state.questions.length)throw new Error('ไม่พบข้อมูลคำถาม');
  };
  ...
  const renderHome=()=>{ui('game-title').textContent=`${state.title} · ${state.questions.length} รูป`;ui('counter').textContent=`1/${state.questions.length}`};
  const render=()=>{const item=state.questions[state.index],total=state.questions.length;ui('counter').textContent=`${state.index+1}/${total}`;ui('round-number').textContent=String(state.index+1).padStart(2,'0');ui('round-label').textContent=`รูปที่ ${state.index+1} / ${total}`;ui('progress').style.width=`${((state.index+1)/total)*100}%`;ui('question').textContent=item.question;ui('image').src=item.image;ui('image').alt=item.alt;ui('answer').textContent=state.revealed?item.answer:'ยังไม่เปิดเฉลย';ui('done-label').textContent=`ครบ ${total} รูปแล้ว`;renderTiles();action('reveal').disabled=state.revealed;action('next').classList.toggle('is-hidden',!state.revealed);action('next').textContent=state.index===total-1?'ดูสรุป':'รูปถัดไป'};
  ...
  const fail=e=>{console.error(e);document.body.innerHTML=`<main style="max-width:680px;margin:40px auto;padding:20px;font-family:system-ui"><h1>โหลดเกมไม่สำเร็จ</h1><p>${e.message}</p><p><a href="/setup.html">ไปหน้าสร้างเกม</a></p></main>`};
